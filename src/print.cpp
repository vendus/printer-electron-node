...
private:
    Napi::Object CreatePrinterObject(Napi::Env env, const PrinterInfo& printer) {
        ...
        if (!success) {  // Se success for true, é um PrintDirect
...
Napi::Value GetStatusPrinter(const Napi::CallbackInfo& info) {
    ...
    // Log do número de argumentos usando %zu para size_t
    printf("Number of arguments: %zu\n", info.Length());

    if (info.Length() < 1 || !info[0].IsObject()) {
        printf("Error: Invalid number of arguments or first argument is not an object\n");
        Napi::TypeError::New(env, "Expected an object as argument").ThrowAsJavaScriptException();
        return env.Null();
    }
